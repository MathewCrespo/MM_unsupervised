...
class LinclsTrainer(object): 
    def __init__(self, net, optimizer, lrsch, train_loader, logger,
                 save_interval=1):
        '''
        Some parameters should be initialized here,
        rather than in the config class in main.py
        for simplicity
        '''
        ...
        ## freeze the backbone and add a lincls layer
    ...
    def train(self,epoch):
        ...
        for img, label in (tqdm(self.train_loader, ascii=True, ncols=60)): # we do not use img label in unsupervised pretrain
            ...
            pred = torch.argmax(output,dim=1) # debug here
            ## for further cls report
